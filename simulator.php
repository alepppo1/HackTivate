...
function getDurationRiskWeight($durationMonths)
{
    if ($durationMonths <= 3) {
        return 0.45; // short-term commitment
    } elseif ($durationMonths <= 12) {
        return 0.75; // manageable commitment
    } elseif ($durationMonths <= 36) {
        return 1.00; // normal long commitment
    } elseif ($durationMonths <= 60) {
        return 1.15; // high long-term pressure
    } else {
        return 1.30; // very long-term commitment
    }
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $durationRiskWeight = getDurationRiskWeight($durationMonths);

    $originalMonthlyAmount = (float) $_POST['monthly_amount'];
    $riskAdjustedMonthlyAmount = $originalMonthlyAmount * $durationRiskWeight;
    ...
    $simCommitment = [
        ...
        "risk_adjusted_amount" => $riskAdjustedMonthlyAmount,
    ...
    // For scoring only, use risk-adjusted amount.
    // For display and saving, keep the real monthly amount.
    $testCommitments[] = [
        "commitment_name" => $simCommitment['commitment_name'],
        "category" => $simCommitment['category'],
        "monthly_amount" => $riskAdjustedMonthlyAmount,
        "duration_text" => $durationText
    ];
    ...
    if ($status == "Safe") {
        ...
        if ($safeLimit > 0) {
            $suggestion = "Avoid or delay this commitment for now. Try reducing the amount to below " . money($safeLimit) . "/month.";
        } else {
    ...
    <main>
        <div class="card" style="margin-bottom:20px;">
            ...
            <div class="grid three" style="margin-top:16px;">
                ...
                <div class="stat">Current Commitment<b><?php echo money($currentHealth['total_new_commitment']); ?></b>
                </div>
                ...
                <form method="POST" class="form">
                    ...
                    <div>
                        ...
                        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
                            <input type="number" name="duration_number" min="1" placeholder="e.g. 12" required>
                    ...
                    <div style="grid-column:1/-1;display:flex;gap:10px;flex-wrap:wrap;">
                        ...
                        <a class="btn secondary" href="commitments.php?profile_id=<?php echo $profile_id; ?>">Back to
                            Commitments</a>
                    </div>
                </form>
            </div>
                ...
                <?php else: ?>
                    <div class="grid two">
                        <div class="scorebox <?php echo getScoreClass($currentHealth['score_after']); ?>">
                            <b>Before</b>
                            <div class="score"><?php echo $currentHealth['score_after']; ?><span
                        ...
                        <div class="scorebox <?php echo getScoreClass($simulatedHealth['score_after']); ?>">
                            <b>After</b>
                            <div class="score"><?php echo $simulatedHealth['score_after']; ?><span
                                    style="font-size:18px;">/100</span></div>
                    ...
                    <div class="feature" style="margin-top:14px;">
                        ...
                        <p class="muted" style="margin-bottom:0;">
                            ...
                            <br>
                            Total commitment value: <?php echo money($simCommitment['total_obligation']); ?>
                            <br>
                            Duration impact: <?php echo $simCommitment['duration_months']; ?> months considered in risk
                            score
                        </p>
                    </div>

                    <div
                        class="coach <?php echo $simulatedHealth['status'] == 'Safe' ? 'good' : ($simulatedHealth['status'] == 'Caution' ? 'warn' : 'bad'); ?>">
                    ...
                    <form method="POST" style="margin-top:14px;display:flex;gap:10px;flex-wrap:wrap;">
                        <input type="hidden" name="commitment_name"
                            value="<?php echo htmlspecialchars($simCommitment['commitment_name']); ?>">
                        <input type="hidden" name="monthly_amount"
                            value="<?php echo htmlspecialchars($simCommitment['monthly_amount']); ?>">
                        <input type="hidden" name="category"
                            value="<?php echo htmlspecialchars($simCommitment['category']); ?>">
                        <input type="hidden" name="duration_number"
                            value="<?php echo htmlspecialchars($_POST['duration_number']); ?>">
                        <input type="hidden" name="duration_unit"
                            value="<?php echo htmlspecialchars($_POST['duration_unit']); ?>">

                        <button class="btn primary" type="submit" name="save_commitment">Save This Commitment</button>
                        <a class="btn secondary" href="simulator.php?profile_id=<?php echo $profile_id; ?>">Try Another
                            Amount</a>
                    </form>
                <?php endif; ?>
            </div>
